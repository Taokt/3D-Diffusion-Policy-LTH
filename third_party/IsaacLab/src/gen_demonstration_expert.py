...
def main():
    ...
    actions_data = np.array(df['policy'].tolist(), dtype=np.float32)

    df['sampled_points'] = df['sampled_points'].apply(ast.literal_eval)
    point_cloud_data = np.array(df['sampled_points'].tolist(), dtype=np.float32)
    ...
    while episode_idx < num_episodes:
        ...
        for i in range(num_data - 1):
            # Extract data from the current and next row
            action = actions_data[i + 1]
            action = np.squeeze(action)
            point_cloud = point_cloud_data[i]
            input_obs_sensor = actions_data[i]
            input_obs_sensor = np.squeeze(input_obs_sensor)

            # Generate random visual and depth data
