...
def main():
    ...
    while episode_idx < num_episodes:
        ...
        while step_count < minimal_episode_length:
            # Generate random observations and actions
            input_obs_visual = np.random.randint(0, 256, size=(3, 84, 84), dtype=np.uint8)  # Random image data
            input_obs_sensor = np.random.rand(24).astype(np.float32)  # Random sensor data
            action = np.random.rand(28).astype(np.float32)  # Random actions
            point_cloud = np.random.rand(1024, 3).astype(np.float32)  # Random point cloud
            depth = np.random.rand(84, 84).astype(np.float32)  # Random depth map
            ...
            depth_arrays_sub.append(depth)

            # Simulate rewards and goal achievements
            total_reward += random.random()  # Random reward
            n_goal_achieved_total += random.choice([0, 1])  # Random goal achieved (binary)
            step_count += 1

        if n_goal_achieved_total < 10.0:
            print(f"Episode {episode_idx} has {n_goal_achieved_total} goals achieved and {total_reward} reward. Discarding.")
        else:
            total_count += total_count_sub
            episode_ends_arrays.append(deepcopy(total_count))  # Index of the last step of the episode
            img_arrays.extend(deepcopy(img_arrays_sub))
            point_cloud_arrays.extend(deepcopy(point_cloud_arrays_sub))
            depth_arrays.extend(deepcopy(depth_arrays_sub))
            state_arrays.extend(deepcopy(state_arrays_sub))
            action_arrays.extend(deepcopy(action_arrays_sub))
            print(f"Episode: {episode_idx}, Reward: {total_reward}, Goal Achieved: {n_goal_achieved_total}")
            episode_idx += 1
